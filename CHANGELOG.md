...
- Environment variable names
  ...
  - Replaced
    ...
    - `F7T_USE_SSL` by `F7T_SSL_USE`
    - `F7T_POLICY_PATH` by `F7T_OPA_POLICY_PATH`
    - `F7T_PERSISTENCE_IP` by `F7T_PERSIST_HOST`
    - `F7T_SSH_CERTIFICATE_WRAPPER` by `F7T_SSH_CERTIFICATE_WRAPPER_USE`
    - `F7T_STATUS_SYSTEMS` by `F7T_SYSTEMS_INTERNAL_STATUS`
    - `F7T_SECRET_KEY` by `F7T_SWIFT_SECRET_KEY`
    - `F7T_USE_SPANK_PLUGIN` by `F7T_SPANK_PLUGIN_USE` 
