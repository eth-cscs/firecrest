...
def get_task(task_id, headers):
	...
	resp = requests.get(url, headers=headers, verify=SSL_SIGNED)
	print(resp.content)
...
def test_post_upload_request(headers):
    ...
    resp = requests.post(STORAGE_URL + "/xfer-external/upload", headers=headers, data=data, verify=SSL_SIGNED)
    ...
    if (OBJECT_STORAGE == "s3v2"):
        ...
        with open(data["sourcePath"], 'rb') as data:
            resp= requests.put(url, data=data, params=params, verify=SSL_SIGNED)
        ...
        resp = requests.post(url, data=post_data, files=files, verify=SSL_SIGNED)
    ...
    download_ok = False
    for i in range(20):
        r = requests.get(TASKS_URL +"/"+task_id, headers=headers, verify=SSL_SIGNED)
...
def test_internal_cp(machine, headers):
    ...
    resp = requests.post(url, headers=headers,data=data, verify=SSL_SIGNED)
    assert resp.status_code == 201
    ...
    resp = requests.get(url, headers=headers, params=params, verify=SSL_SIGNED)
    print(resp.json())
    print(machine)
    assert resp.status_code == 200
