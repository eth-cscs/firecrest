...
def download_task(headers, system_name, system_addr, sourcePath, task_id):
    ...
    # upload starts:
    res = exec_remote_command(headers, system_name, system_addr, upload_url["command"], log_command=upload_url["command"])
...
def internal_operation(request, command):
    ...
    try:
        ...
        if USE_SCHED_PROJECT:
            ...
            resp = exec_remote_command(headers, system_name, system_addr, id_command, log_command="id")
            if resp["error"] != 0:
    ...
    resp = exec_remote_command(headers, system_name, system_addr, f"ID={ID} true", log_command=command)
