...
AUTH_REQUIRED_SCOPE = os.environ.get("F7T_AUTH_REQUIRED_SCOPE",
                                     '').strip('\'"')
...
#   Difference to other microservices: allow '>' for 'cat' and 'head', '&' for
#   Storage URLs, single quotes (') for arguments
...
OPA_POLICY_PATH = os.environ.get("F7T_OPA_POLICY_PATH",
                                 "v1/data/f7t/authz").strip('\'"')
...
if JAEGER_AGENT != "":
    config = Config(
        config={'sampler': {'type': 'const', 'param': 1},
                'local_agent': {'reporting_host': JAEGER_AGENT,
                                'reporting_port': 6831},
                'logging': True,
                'reporter_batch_size': 1}, service_name="certificator")
...
class LogRequestFormatter(logging.Formatter):
    ...
    def format(self, record):
        try:
            # try to get TID from Flask g object, it's set on
            # @app.before_request on each microservice
            record.TID = g.TID
        except Exception:
            try:
...
def setup_logging(logging, service):
    ...
    if LOG_TYPE == "file":
        ...
        logHandler = TimedRotatingFileHandler(f'{LOG_PATH}/{service}.log',
                                              when='D', interval=1)
...
def check_key_permission():
    ...
    try:
        ...
        if oct(cas & 0o477) != '0o400':
            msg = f"ERROR: wrong '{CA_KEY_PATH}' permissions,"
            " please set to 400. Exiting."
            app.logger.error(msg)
            sys.exit(msg)
    except OSError as e:
        msg = f"ERROR: couldn't stat '{CA_KEY_PATH}', message: {e.strerror}"
        " - Exiting."
        app.logger.error(msg)
        sys.exit(msg)
...
def check_user_auth(username, system):
    ...
    if OPA_ENABLED:
        ...
        try:
            resp_opa = requests.post(f"{OPA_URL}/{OPA_POLICY_PATH}",
                                     json=input,
            ...
            if not resp_opa.ok:
                return {"allow": False,
                        "description": f"Server error: {msg}",
                        "status_code": resp_opa.status_code}
...
def check_header(header):
    ...
    if not is_public_key_set:
        ...
        try:
            ...
            # only check for expired signature or general exception for this
            # case
    ...
    # if token was successfully decoded, then check if required scope is
    # present
